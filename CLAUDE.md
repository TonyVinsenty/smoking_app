...
Next: quick owner fixes FIRST: 1) Settings header «Оформление» → «Цветовой тон», options in order «Системный · Тёмный · Светлый»; 2) SOS FAB slightly transparent (it covers the quote card on Home; also keep enough bottom padding); 3) Home timer tiles (months/weeks/days) visually merge — add separators between them (e.g. thin vertical dividers or «·»). Then SOS rework (owner decision): anxious users panic from breath-holding → replace current in/hold/out exercise with 3 choices: «Спокойное дыхание» (in 4 / out 6, no hold, soft growing circle; default), «Отсчёт 10 → 0» (big numbers every 2 s, no commands), «Заземление 5-4-3-2-1» (step cards: 5 see, 4 hear, 3 touch, 2 smell, 1 taste). Small hint at top of SOS «Упражнение можно сменить» opening a picker; same choice in Settings (prefs). Then «Я закурил» flow (confirm + diary) + attempt history → Knowledge tab → Settings «Мои данные» (edit products) → notifications → craving-map stats.
